module Support
  class Cases::ReviewEvaluationsController < Cases::ApplicationController
    ...
    before_action :set_revoked_template
    ...
    def update
      ...
      send_evaluations_revoked_email
  ...
  private
    ...
    def default_revoked_email_template = render_to_string(partial: "support/cases/review_evaluations/default_revoked_email_template")
    ...
    def set_revoked_template
      @revoked_template = Support::EmailTemplate.find_by(title: "Email notification of evaluation being revoked")
      @revoked_template_id = @revoked_template.id if @revoked_template
    end
    ...
    def send_evaluations_approved_email
      ...
      if pending_evaluatons.any?
        pending_evaluatons.each do |evaluator_email|
          draft_and_send_email(evaluator_email.to_json, default_approved_email_template, default_approved_email_subject, @approved_template, @approved_template_id)
        end
      end
    end

    def send_evaluations_revoked_email
      filtered_params = filter_blank_evaluations(review_evaluations_params)
      reset_evaluation_emails = @approved_evaluations - filtered_params[:evaluation_approved]

      if reset_evaluation_emails.any?
        reset_evaluation_emails.each do |evaluator_email|
          draft_and_send_email(evaluator_email.to_json, default_revoked_email_template, default_revoked_email_subject, @revoked_template, @revoked_template_id)
        end
      end
    end

    def draft_and_send_email(to_recipients, default_content, default_subject, template, template_id)
