    ...
    def index
      respond_to do |format|
        ...
        format.html do
          ...
          @my_cases = FilterCases.new.filter(params[:filter_my_cases_form]).where.not(state: :closed).by_agent(current_agent&.id).my_cases_ordering.map { |c| CasePresenter.new(c) }.paginate(page: params[:my_cases_page])
        end
      end
    end
