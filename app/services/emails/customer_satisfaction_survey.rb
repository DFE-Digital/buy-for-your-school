...
private
  ...
  def template_params
    super.merge(**recommendations_links, **satisfaction_level_links, caseworker_name:)
  end
  ...
  def recommendations_links
    links = {}
    (0..10).map { |k| links["#{k}_link"] = Rails.application.routes.url_helpers.edit_customer_satisfaction_surveys_recommendation_likelihood_url(@survey_id, customer_satisfaction_survey: { recommendation_likelihood: k.to_s }) }
    links.symbolize_keys
  end
end
