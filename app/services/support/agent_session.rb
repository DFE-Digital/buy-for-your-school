...
#
# @see Support::SessionsController
module Support
  class AgentSession
    ...
    # Remove agent uidn from the session
    #
    # @return [nil]
    def delete!
      @session.delete(:dfe_support_sign_in_uid)
      @session.delete(:dfe_support_sign_in_sign_out_token)
    end
    ...
    def should_be_signed_out_of_dsi?
      @session[:dfe_support_sign_in_sign_out_token].present?
    end
    ...
    def sign_out_url
      ...
      query = {
        id_token_hint: @session[:dfe_support_sign_in_sign_out_token],
    ...
    #
    def persist_successful_dfe_support_sign_in_claim!(auth:)
      @session[:dfe_support_sign_in_uid] = auth.fetch("uid")
      @session[:dfe_support_sign_in_sign_out_token] = auth.dig("credentials", "id_token")
    end
    ...
    # @param auth [OmniAuth::AuthHash]
    #
    def invalidate_other_agent_sessions(auth:)
      ...
      session_lookup_key = "agent_dsi_id:#{dfe_sign_in_uid}"
