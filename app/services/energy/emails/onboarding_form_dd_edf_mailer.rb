module Energy
  ...
  private
    ...
    def email_template
      config_template = Energy::EmailTemplateConfiguration.find_by(energy_type: :electricity, configure_option: :school_electricity_direct_debit_template)
      @email_template ||= if config_template
                            Support::EmailTemplate.find(config_template.support_email_templates_id)
                          else
                            Support::EmailTemplate.find_by(title: FORM_DD_EDF_EMAIL_TEMPLATE, archived: false)
                          end
    end
