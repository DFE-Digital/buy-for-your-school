  ...
  describe "#available_categories" do
    ...
    context "when a tower is provided" do
      it "only returns the categories in that tower" do
        expect(available_categories(tower).map(&:title)).to contain_exactly("All", "Tower Cat 1", "Tower Cat 2")
      end
    end

    context "when no tower is provided" do
      it "returns all categories" do
        expect(available_categories.map(&:title)).to contain_exactly("All", "Tower Cat 1", "Tower Cat 2", "Non-Tower Cat 1", "Non-Tower Cat 2")
      end
    end
  end
end
