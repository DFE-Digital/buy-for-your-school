...
RSpec.describe School::RecordKeeper do
  ...
  describe "legacy_record" do
    context "with a school that is closed" do
      context "and has no existing record" do
        it "returns true" do
          expect(record_keeper.send(:legacy_record?, legacy_record)).to eq(true)
        end
      end

      context "and has an existing record" do
        before { create(:support_organisation, urn: legacy_record[:urn]) }

        it "returns false" do
          expect(record_keeper.send(:legacy_record?, legacy_record)).to eq(false)
        end
      end
    end

    context "with a school that is open" do
      context "and has no existing record" do
        it "returns false" do
          expect(record_keeper.send(:legacy_record?, new_record)).to eq(false)
        end
      end

      context "with a school that is open and has an existing record" do
        before { create(:support_organisation, urn: new_record[:urn]) }
  ...
  describe "#parse_opened_date" do
    context "with a blank date" do
      it "returns nil" do
        expect(record_keeper.send(:parse_opened_date, "")).to be_nil
      end
    end

    context "with nil" do
      it "returns nil" do
        expect(record_keeper.send(:parse_opened_date, nil)).to be_nil
      end
    end

    context "with a date with the format DD-MM-YYYY" do
      it "returns a Time object" do
        expect(record_keeper.send(:parse_opened_date, "01-02-2001")).to eq(Time.zone.parse("1/2/2001"))
      end
    end
  end
end
