...
describe "Agent can review evaluations", :js, :with_csrf_protection do
  ...
  specify "When agent approve correctly completed evaluations" do
    ...
    expect(page).to have_content("View Momo Taro's evalaution files")

    click_button "Continue"

    expect(page).to have_content("There is a problem")
    expect(page).to have_content("Select at least one evaluation to approve")
    ...
    expect(page).not_to have_content("Select at least one evaluation to approve")
