RSpec.feature "Case procurement details" do
  ...
  before do
    ...
    click_on "Procurement details"
  end

  context "when there are no procurement details" do
    it "does not have a change link" do
      within("div#procurement-details") do
        expect(all("h2.govuk-heading-m")[0]).to have_text "Procurement details"
        expect(all("h2.govuk-heading-m")[0]).not_to have_link "change"
      end
    end

    it "shows blank fields" do
      within("div#procurement-details") do
        expect(all("dt.govuk-summary-list__key")[0]).to have_text "Required agreement type"
        expect(all("dt.govuk-summary-list__value")[0]).to have_text ""
        expect(all("dt.govuk-summary-list__key")[1]).to have_text "Route to market"
        expect(all("dt.govuk-summary-list__value")[1]).to have_text ""
        expect(all("dt.govuk-summary-list__key")[2]).to have_text "Reason for route to market"
        expect(all("dt.govuk-summary-list__value")[2]).to have_text ""
        expect(all("dt.govuk-summary-list__key")[3]).to have_text "Framework name"
        expect(all("dt.govuk-summary-list__value")[3]).to have_text ""
        expect(all("dt.govuk-summary-list__key")[4]).to have_text "Procurement start date"
        expect(all("dt.govuk-summary-list__value")[4]).to have_text ""
        expect(all("dt.govuk-summary-list__key")[5]).to have_text "Procurement end date"
        expect(all("dt.govuk-summary-list__value")[5]).to have_text ""
        expect(all("dt.govuk-summary-list__key")[6]).to have_text "Procurement stage"
  ...
  context "when there are procurement details" do
    context "and they are blank" do
      ...
      it "has a change link" do
        within("div#procurement-details") do
          expect(all("h2.govuk-heading-m")[0]).to have_text "Procurement details"
          expect(all("h2.govuk-heading-m")[0]).to have_link "change", href: "/support/cases/#{support_case.id}/procurement_details/edit", class: "govuk-link"
        end
      end
