  ...
  context "when there are no procurement details" do
    it "does not have a change link" do
      within("div#case-details") do
        expect(all("h3.govuk-heading-m")[1]).to have_text "Procurement details"
        expect(all("h3.govuk-heading-m")[1]).not_to have_link "change"
      end
    end
  end
  ...
  context "when there are procurement details" do
    context "and they are blank" do
      ...
      it "has a change link" do
        within("div#case-details") do
          expect(all("h3.govuk-heading-m")[1]).to have_text "Procurement details"
          expect(all("h3.govuk-heading-m")[1]).to have_link "change", href: "/support/cases/#{support_case.id}/procurement_details/edit", class: "govuk-link"
        end
      end
    end
