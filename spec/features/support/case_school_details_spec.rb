describe "Case school details" do
  ...
  context "when open case" do
    let(:support_case) { create(:support_case, :opened, organisation: support_organisation, first_name: "John", last_name: "Smith", email: "john.smith@email.com") }
    ...
    it "has change links for contact details and organisation" do
      within "#school-details" do
        ...
        expect(all("dt.govuk-summary-list__key")[2]).to have_text "Contact email"
        expect(all("dd.govuk-summary-list__value")[2]).to have_text "john.smith@email.com"
        expect(all("dd.govuk-summary-list__actions")[2]).to have_link "Change"

        expect(all("dt.govuk-summary-list__key")[3]).to have_text "Organisation name"
        expect(all("dd.govuk-summary-list__value")[3]).to have_text "School #1"
  ...
  context "when closed case" do
    let(:support_case) { create(:support_case, state: :closed, organisation: support_organisation, first_name: "Bill", last_name: "Jones", email: "bill.jones@email.com") }
    ...
    it "has no change links" do
      within "#school-details" do
        ...
        expect(all("dt.govuk-summary-list__key")[2]).to have_text "Contact email"
        expect(all("dd.govuk-summary-list__value")[2]).to have_text "bill.jones@email.com"
