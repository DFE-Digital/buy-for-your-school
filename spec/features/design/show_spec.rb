    ...
    specify "all the steps in a category" do
      ...
      within("ol.govuk-list.govuk-list--number") do
        ...
        within(list_items[0]) do
          expect(find("code#liquid_tag")).to have_text "{{ answer_radio-question_types }}"

          expect(page).to have_link "Which service do you need?",
                                    href: "https://app.contentful.com/spaces/jspwts36h1os/environments/master/entries/radio-question_types",
                                    class: "govuk-link"
        end

        within(list_items[1]) do
          expect(find("code#liquid_tag")).to have_text "{{ answer_short-text-question_types }}"

          expect(page).to have_link "What email address did you use?",
                                    href: "https://app.contentful.com/spaces/rwl7tyzv9sys/environments/master/entries/short-text-question_types",
                                    class: "govuk-link"
        end

        within(list_items[2]) do
          expect(find("code#liquid_tag")).to have_text "{{ answer_long-text-question_types }}"

          expect(page).to have_link "Describe what you need",
                                    href: "https://app.contentful.com/spaces/rwl7tyzv9sys/environments/develop/entries/long-text-question_types",
                                    class: "govuk-link"
        end

        within(list_items[3]) do
          expect(find("code#liquid_tag")).to have_text "{{ answer_checkboxes-question_types }}"

          expect(page).to have_link "Everyday services that are required and need to be considered",
                                    href: "https://app.contentful.com/spaces/rwl7tyzv9sys/environments/develop/entries/checkboxes-question_types",
                                    class: "govuk-link"
        end
      end
    end
  ...
  context "when the map isn't valid" do
    ...
    describe "the same entry is found twice" do
      it "returns an error message" do
        ...
        # errors.repeat_step_in_the_contentful_journey.page_body, entry_id: "radio-question_types"
        expect(find("p.govuk-body")).to have_text <<~DUPLICATE.chomp
          One or more steps in the Contentful journey would leave the user in an infinite loop. This entry ID was presented more than once to the user: radio-question_types
        DUPLICATE
      end
    end
  end
