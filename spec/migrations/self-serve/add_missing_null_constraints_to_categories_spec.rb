...
RSpec.describe AddMissingNullConstraintsToCategories do
  ...
  context "when creating a Category without a description" do
    it "fails due to null constraint" do      
      expect { up }.to change { Category.reset_column_information; Category.columns.find { |c| c.name == "description" }.null }.from(true).to(false)
  ...
  context "when creating a Category without a slug" do
    it "fails due to null constraint" do      
      expect { up }.to change { Category.reset_column_information; Category.columns.find { |c| c.name == "slug" }.null }.from(true).to(false)
