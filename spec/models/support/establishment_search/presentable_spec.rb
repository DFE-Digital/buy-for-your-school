...
describe Support::EstablishmentSearch::Presentable do
  ...
  describe "#autocomplete_template" do
    context "when the search result is an opening school" do
      it "returns the correct autocomplete template" do
        search_result = presentable.omnisearch(opening_school.urn).to_a.first
        expect(search_result.autocomplete_template).to eq("<strong>Test Opening School</strong>, EC3A 5DE<br />Foundation school, Proposed to open 30-07-2024<br />URN: 3 - UKPRN: 333\n")
      end
    end

    context "when the search result is a closing school" do
      it "returns the correct autocomplete template" do
        search_result = presentable.omnisearch(closing_school.urn).to_a.first
        expect(search_result.autocomplete_template).to eq("<strong>Test Closing School</strong>, EC3A 5DE<br />Academy sponsor led, Open, but proposed to close 30-08-2024<br />URN: 2 - UKPRN: 222\n")
      end
    end
