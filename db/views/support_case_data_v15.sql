SELECT
  ...
  sir.additional_data ->>'referrer' AS referrer
...
LEFT JOIN frameworks_frameworks ff
  ON sp.frameworks_framework_id = ff.id
LEFT JOIN support_contracts ec
  ON sc.existing_contract_id = ec.id
LEFT JOIN support_contracts nc
  ON sc.new_contract_id = nc.id
  LEFT JOIN
    support_interactions AS ps
    ON ps.id =
    (
        SELECT id
        FROM support_interactions i
        WHERE i.event_type = 3 and i.case_id = sc.id
        AND i.additional_data ->>'email_template' = 'fd89b69e-7ff9-4b73-b4c4-d8c1d7b93779'
        ORDER BY i.created_at ASC
        LIMIT 1
    ) 
    LEFT JOIN
    support_interactions AS es
    ON es.id =
    (
        SELECT id
        FROM support_interactions i
        WHERE  i.case_id = sc.id and
        i.body in  ('The exit survey email has been sent', 'The survey for resolved cases email has been sent')
        ORDER BY i.created_at ASC
        LIMIT 1
    )
  LEFT JOIN
    support_interactions AS sir
    ON sir.id =
    (
        SELECT id
        FROM support_interactions i
