...
LEFT JOIN (
	...
	stc.procurement_id,
	stc.tower_name
	FROM public.support_tower_cases stc
) as stc
...
(
	SELECT id
	FROM public.support_activity_log_items first_resolved
...
(
	SELECT id
	FROM public.support_activity_log_items last_resolved
...
LEFT JOIN (
	...
	fr.support_case_id,
	fr.school_urns
	FROM public.framework_requests fr
) as fr
...
ON si.id =
  (
    SELECT id
    FROM public.support_interactions i
...
LEFT JOIN (
  ...
  organisations.urn,
  organisations.ukprn,
  organisations.trust_name as trust_name,
  ...
  FROM public.support_organisations organisations
  ...
  JOIN support_establishment_groups parent
  	ON parent.uid = organisations.trust_code
  UNION ALL
  ...
  FROM public.support_establishment_groups egroups
