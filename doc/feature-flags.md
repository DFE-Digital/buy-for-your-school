...
|auto_email_vat_dd|when enabled, auto email sending of emails regarding DD/VAT|DISABLED|To be enabled when required
...
|sc_tasklist_case|The task list tab for a case at level 4 or 5 will be visible, and the 'case action_required' flag will be updated based on the evaluation document upload status and the evaluation approval status. |ENABLED |Feature now live, flag to be removed|
